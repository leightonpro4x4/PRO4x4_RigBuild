...
export function bootstrap(origin){
  ...
  const store=new ProjectStore({filename:path.join(directory,'alpha94.sqlite'),catalogue:json('subsystems/catalogue/catalogue.json'),fixture:json('subsystems/catalogue/alpha93-fixture.json'),mapping:json('subsystems/catalogue/alpha93-mapping.json'),assetHashes:Object.fromEntries(report.deploymentAssets.map(a=>[a.file,a.sha256])),implementationVersion:hash(['subsystems/domain/engine.mjs','subsystems/visual-eligibility/adapter.mjs','subsystems/visual-runtime/ranger.mjs','subsystems/catalogue/alpha93-mapping.json','subsystems/catalogue/alpha93-fixture.json','subsystems/server-persistence/store.mjs'].map(read))});
  return {store,api:createAPI(store,{origin,finaliserToken:process.env.PRO4X4_FINALISER_TOKEN,finaliserId:process.env.PRO4X4_FINALISER_ID})};
}
