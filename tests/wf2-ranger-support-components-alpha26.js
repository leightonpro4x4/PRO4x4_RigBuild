...
// Manufacturer category placement alone is not enough. The current Type X EVO product-page fitment table omits Ranger RA 2022-on, so the SKU may be governed only as an engineering/staff-review route, never as confirmed Next Gen Ranger fitment.
const evo=byId.get('oa-rally-hoop-stedi-evo-ranger');
assert(evo,'Type X EVO 8.5 hoop may be present only as a governed unresolved route');
assert.equal(evo.sku,'TB-COM-RAL-STE-2XEVO-ASM0');
assert.equal(evo.status,'engineering');
assert.equal(evo.fitment?.reviewRequired,true);
assert.equal(evo.visual?.status,'staff-review');
...
console.log(JSON.stringify({schemaVersion:d.schemaVersion,accessories:d.accessories.length,newConfirmedRecords:Object.keys(expected).length,evidenceRows:ev.sources.length,engineeringForMissingExactFitment:['TB-COM-RAL-STE-2XEVO-ASM0'],unknownInstallPreserved:['oa-rally-hoop-stedi-pro-ranger','oa-rally-hoop-9in-ranger','oa-rally-hoop-7in-ranger','oa-22in-slim-lightbar-ranger','oa-deep-dish-floor-mats-ranger'],status:'pass'},null,2));
