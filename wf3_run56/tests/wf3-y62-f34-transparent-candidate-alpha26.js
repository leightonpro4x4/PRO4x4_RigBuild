...
(function(){
  const c=cands.listHistory('front34').find(x=>x.candidateId==='Y62-F34-V1-CANDIDATE-02');
  ...
  const cam=cams.get('front34');assert.equal(cam.transparentCandidate.id,'Y62-F34-V1-CANDIDATE-04');assert.equal(cam.transparentCandidate.hasAlpha,true);assert.equal(cam.transparentCandidate.productionEligible,false);assert.equal(cam.transparentCandidate.cameraMatched,false);
  assert.equal(readiness.canonicalMasters.front34.latestCandidateId,'Y62-F34-V1-CANDIDATE-04');assert.equal(readiness.canonicalMasters.front34.productionEligible,false);
  ...
  const reviewHtml=fs.readFileSync(path.join(root,'y62-canonical-review.html'),'utf8');assert.match(reviewHtml,/Candidate 04/);assert.match(reviewHtml,/REVIEW DECISION: RETURN/);assert.match(reviewHtml,/NOT CUSTOMER VISIBLE/);
