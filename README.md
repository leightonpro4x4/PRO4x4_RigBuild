...
Stage 8 root CI: see consolidation/STAGE_8.md and validation.config.json. `npm test` runs all nine validation groups; `npm run test:core` runs the JavaScript groups. Install exact Node/npm/Python versions from .node-version, package.json and .python-version, then `npm ci`, create .venv, install requirements-validation.lock with --require-hashes --only-binary=:all:, and `npx playwright install chromium`. Normal Windows developers can run tools/validate-windows.ps1. Generated results are in ignored .validation/results, not historical reports.
