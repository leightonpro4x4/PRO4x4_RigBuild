...
try{
  ...
  const reviewed={...staged.payload,
    provenance:{...staged.payload.provenance,sourceType:'reference-derived-canonical',licenceStatus:'owned',licenceNote:'WF5 synthetic QA fixture; canonical master is explicitly backed by the locked owner reference pack.',referenceIds:[...briefs.briefs.front34.referenceIds]},
  ...
  assert.deepEqual(promoted.asset.provenance.referenceIds,briefs.briefs.front34.referenceIds);
  for(const id of promoted.asset.provenance.referenceIds)assert.ok(referencePack.references.some(r=>r.id===id&&r.rights==='owner-project-approved'),`approved owner reference missing: ${id}`);
  ...
  console.log(JSON.stringify({
    ...
    referenceBackedPositiveControl:true,
