...
const expected={
 ...
 'oa-toro-indicator-harness-ranger':['LM-FRA-NG-IND',80,1],
...
for(const [id,[sku,price,weight]] of Object.entries(expected)){
 const p=byId.get(id);assert(p,id);assert.equal(p.sku,sku);assert.equal(p.price,price);assert.equal(p.weightKg,weight);assert.equal(p.fitment?.source,'manufacturer');assert(/^https:\/\/offroadanimal\.com\.au\//.test(p.sourceUrl));
 const e=ev.sources.find(x=>x.id===id);assert(e,`missing source evidence for ${id}`);assert.equal(e.sku,sku);assert.equal(e.rrpAud,price);
}
...
const harness=byId.get('oa-toro-indicator-harness-ranger');assert.deepEqual(harness.requires,['oa-toro-ranger']);assert.equal(harness.installTimeHours,null);
