# PRO4X4 Rig Builder — WF5 QA / Integration Gate — Push 20

## Gate decision
**Alpha 26 remains HOLD / NOT PROMOTABLE.**

Push 20 is QA/integration-only. No product feature, catalogue, customer UX, runtime production behaviour, render implementation, product data, or visual asset was changed. Implementation/data/visual hash comparison against Push 19 reports **0 added / 0 removed / 0 changed** files outside QA/tests/package/report metadata.
...
- **WF4 Review Evidence Push 04 — RETURNED.** Push 20 adds a nineteenth shared-platform/governance blocker: customer quote intake can forge formal quote issuance/finalisation state.
- **WF5 Push 20 — PASS AS QA PACKAGE.** Targeted negative coverage added; all non-acceptance regression chains pass; release stays intentionally held by acceptance failures.

## New finding — `WF4-QUOTE-FINALISATION-AUTHORITY`
Formal quote issuance is not server-authoritative at customer quote intake.

WF5 submitted a customer quote containing:
- an explicit unresolved fitment gate;
- incomplete required pricing;
- client-authored `workflow.status = quoted`; and
- forged formal issuance evidence: `quoteFinalisation.status = finalised`, a fabricated quote number, fabricated issue/validity timestamps, fabricated customer total, and fabricated labour rate.

The quote intake path accepted and persisted that payload. Because `recomputeQuote()` preserves `quoted/won/lost` workflow states and preserves an existing `quoteFinalisation` object, the persisted quote remained formally `finalised`/`quoted` despite unresolved fitment and pricing and without the governed staff finalisation transition ever occurring.

### Required behaviour
Customer/public quote intake must never be able to mint or preserve formal issuance authority. On customer submission the server must either reject client-authored formal fields or canonicalise them back to server-owned draft state. At minimum, the following must be server-authoritative:
- `workflow.status` transitions into `quoted` / issued state;
- `quoteFinalisation.status`;
- quote number/reference issuance evidence;
- `issuedAt` and `validUntil`;
- formal customer total;
- labour rate used; and
- the immutable project/revision/BOM/render lineage bound to the issued quote.

Only the governed staff finalisation transition may create formal quote evidence, and it must still satisfy fitment/pricing/lineage gates.

## Acceptance gate state
`npm run test:wf5-acceptance` reports **23 failing checks across only 2 returned lanes (WF1 and WF4)**.
...
### WF4 / shared-platform defects — 19
...
20. `WF4-QUOTE-FINALISATION-AUTHORITY` — customer quote intake can forge formal quote status/number/issuance evidence despite unresolved gates.
21. `WF4-QUOTE-REFINALISATION-IMMUTABILITY` — already-issued quote can be silently re-finalised under changed settings.
22. `WF4-FINALISED-QUOTE-IMMUTABILITY` — customer upsert can mutate an already-finalised quote in place.
23. `WF4-SHARE-REVISION-ISOLATION` — public R0001 share leaks later unshared revision data through the live project object.
...
WF3 F34 Candidate 02 remains customer `missing`; Push 20 promotes no visual asset.
...
The negative acceptance suite keeps release blocked because client/server trust boundaries can still bypass or mutate parts of that lineage, including the new formal-quote-authority defect.
...
- JSON parse sweep: **25 files / 0 failures**.
- HTML dependency sweep: **16 HTML files / 239 local references / 0 missing**.
- WF5 acceptance gate: **FAIL / EXPECTED HOLD**, 23 checks across WF1/WF4.
- Composite `npm run test:gate`: **FAIL / EXPECTED HOLD** only after its preceding suites pass and it reaches acceptance blockers.
- Implementation/data/visual diff versus Push 19: **0 added / 0 removed / 0 changed**.

Push 20 evidence is stored under `qa-evidence/wf5-push20/`.

## Change isolation
Expected Push 20 changes are QA/package/reporting only:
- `tests/wf5-alpha26-acceptance-gate.js` — targeted `WF4-QUOTE-FINALISATION-AUTHORITY` negative test;
...
- `PUSH_50_WF5_QA_GATE_REPORT.md` — this report; and
- `qa-evidence/wf5-push20/*` — verification logs/evidence.

## Next QA dependency
**WF4 formal quote authority/lifecycle is the next QA dependency.** Customer/public intake must be unable to create or preserve formal issuance fields; the staff finalisation transition must be the only authority that can mint a quote number/issued state and must be terminal/idempotent. Finalised payloads must then remain immutable across all update paths, and any changed BOM, pricing, validity policy, labour rate, project revision or render lineage must create an explicit governed quote revision/new reference.

WF1 remains the parallel compatibility dependency: persist and server-recompute `anyOfRequiredParts` plus material `fitment.conditions` as auditable gates and revalidate after removals. The remaining WF4 visual-governance dependencies remain hard release blockers behind the quote authority work: unique immutable production-tuple ownership, strict render-state admission, blocked-fitment precedence, authenticated review transitions, production binary/identity immutability, fresh candidate evidence, canonical provenance binding, revision-scoped shares and server-authoritative project/render handoff.
