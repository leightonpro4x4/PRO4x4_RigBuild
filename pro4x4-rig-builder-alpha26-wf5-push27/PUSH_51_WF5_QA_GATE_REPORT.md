# PRO4X4 Rig Builder — WF5 QA / Integration Gate — Push 21

## Gate decision
**Alpha 26 remains HOLD / NOT PROMOTABLE.**

Push 21 is QA/integration-only. No product feature, catalogue record, customer UX, runtime production behaviour, render implementation, product data, or visual asset was changed. Implementation/data/visual hash comparison against Push 20 reports **0 added / 0 removed / 0 changed** files outside QA/tests/package/report metadata.
...
- **WF4 Review Evidence Push 04 — RETURNED.** Push 21 adds a twentieth shared-platform/governance blocker: a formal quote can be issued without any existing immutable project/revision lineage.
- **WF5 Push 21 — PASS AS QA PACKAGE.** Targeted negative coverage added; all non-acceptance regression chains pass; release stays intentionally held by acceptance failures.

## New finding — `WF4-QUOTE-LINEAGE-REQUIRED`
Formal quote issuance does not currently require an existing immutable project revision.

WF5 submitted a complete, gate-clear customer quote snapshot directly through the quote intake contract with **no `project.id` and no `project.revisionId`**. The staff finalisation path then issued it successfully as formal quote `Q-WF5-UNBOUND-FORMAL`.

That leaves the issued commercial document without an authoritative immutable project snapshot to prove which BOM, pricing inputs, render lineage and customer configuration were actually approved. It also creates a second path around the merged customer flow, which normally saves an immutable project revision before queueing the quote.

### Required behaviour
Draft lead/quote intake may remain permissive if required for backwards compatibility, but **formal issuance must be lineage-bound**. Before `quoteFinalisation.status = finalised` can be created, the server must:
- require `quote.project.id` and `quote.project.revisionId`;
- verify both resolve to an existing immutable stored project revision;
- verify or canonicalise the quote BOM, commercial values, gates and render evidence from that exact revision; and
- persist the verified project/revision identity into the issued quote evidence.

If no valid immutable revision exists, finalisation must fail. A client-authored or non-existent lineage identifier must not be enough.

This finding complements, rather than duplicates, `WF4-QUOTE-PROJECT-REVISION-BINDING`: that earlier check proves a quote **claiming** a real revision can carry different content. Push 21 proves a formal quote can currently be issued with **no immutable revision at all**.

## Acceptance gate state
`npm run test:wf5-acceptance` reports **24 failing checks across only 2 returned lanes (WF1 and WF4)**.
...
### WF4 / shared-platform defects — 20
...
21. `WF4-QUOTE-FINALISATION-AUTHORITY` — customer quote intake can forge formal quote status/number/issuance evidence despite unresolved gates.
22. `WF4-QUOTE-REFINALISATION-IMMUTABILITY` — already-issued quote can be silently re-finalised under changed settings.
23. `WF4-FINALISED-QUOTE-IMMUTABILITY` — customer upsert can mutate an already-finalised quote in place.
24. `WF4-SHARE-REVISION-ISOLATION` — public R0001 share leaks later unshared revision data through the live project object.
...
WF3 F34 Candidate 02 remains customer `missing`; Push 21 promotes no visual asset.
...
The negative acceptance suite keeps release blocked because server trust boundaries can still bypass or mutate that lineage. Push 21 closes a missing QA case at the front of formal issuance: a final quote currently does not have to possess a project/revision lineage at all.
...
- WF5 acceptance gate: **FAIL / EXPECTED HOLD**, 24 checks across WF1/WF4.
- Composite `npm run test:gate`: **FAIL / EXPECTED HOLD** only after its preceding suites pass and it reaches acceptance blockers.
- Implementation/data/visual diff versus Push 20: **0 added / 0 removed / 0 changed**.

Push 21 evidence is stored under `qa-evidence/wf5-push21/`.

## Change isolation
Expected Push 21 changes are QA/package/reporting only:
- `tests/wf5-alpha26-acceptance-gate.js` — targeted `WF4-QUOTE-LINEAGE-REQUIRED` negative test;
...
- `PUSH_51_WF5_QA_GATE_REPORT.md` — this report; and
- `qa-evidence/wf5-push21/*` — verification logs/evidence.

## Next QA dependency
**WF4 formal quote authority + immutable lineage is the next QA dependency.** A formal quote must first resolve to a real immutable project/revision and be canonicalised against that revision. The governed staff finalisation transition must then be the sole authority that can mint formal issuance evidence, and that issued state must be terminal/idempotent and immutable across all update paths. Any changed BOM, pricing, validity policy, labour rate, project revision or render lineage must create an explicit governed quote revision/new reference rather than rewriting the issued document.
