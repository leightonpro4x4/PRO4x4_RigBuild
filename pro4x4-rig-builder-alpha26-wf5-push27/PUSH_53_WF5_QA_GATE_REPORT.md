# PRO4X4 Rig Builder — WF5 QA / Integration Gate — Push 23

## Gate decision
**Alpha 26 remains HOLD / NOT PROMOTABLE.**

Push 23 is QA/integration-only. No product feature, catalogue record, customer UX, runtime production behaviour, render implementation, product data, or visual asset was changed. Implementation/data/visual hash comparison against Push 22 reports **0 added / 0 removed / 0 changed** files outside QA/tests/package/report metadata.
...
- **WF4 Review Evidence Push 04 — RETURNED.** Push 23 adds a twenty-second shared-platform/governance blocker: a customer can bind a quote to an immutable project revision owned by another customer and still reach formal issuance.
- **WF5 Push 23 — PASS AS QA PACKAGE.** Targeted negative coverage added; all non-acceptance regression chains pass; release stays intentionally held by acceptance failures.

## New finding — `WF4-QUOTE-LINEAGE-OWNERSHIP`
A real, existing immutable project/revision tuple is still insufficient lineage evidence if the submitting customer is **not authorised for that project**.

WF5 created `P4X4-PROJ-WF5-LINEAGE-OWNER/R0001` under customer **`wf5-lineage-owner-a`**. The existing project access control correctly rejects direct `getProject()` access by **`wf5-lineage-owner-b`**. However, Customer B was able to submit the exact saved R0001 snapshot through quote intake while preserving Customer A's project/revision identifiers. The quote was accepted and the staff finalisation path issued it formally.

This proves the quote path currently bypasses the ownership boundary already enforced by the project path. A quote can therefore claim a legitimate immutable lineage tuple that belongs to a different customer.

### Required behaviour
Before quote intake accepts persisted project lineage, and again before formal issuance, the server must:
- resolve the claimed project and exact immutable revision from authoritative storage;
- verify the submitting customer is authorised for that project, or require an explicit governed staff transfer/representative action;
- bind the authorised customer/project/revision relationship into quote provenance/audit evidence;
- reject foreign project/revision lineage even when the snapshot payload is byte-for-byte consistent with the real revision; and
- preserve that authorisation decision through staff finalisation rather than treating administrator authority as retroactive customer consent.

This is distinct from the three existing quote-lineage defects:
- `WF4-QUOTE-LINEAGE-REQUIRED` — formal issuance can occur with **no** immutable project/revision lineage;
- `WF4-QUOTE-LINEAGE-EXISTENCE` — formal issuance can occur with a **fabricated/non-existent** revision; and
- `WF4-QUOTE-PROJECT-REVISION-BINDING` — a quote can claim a **real revision while carrying different content**.

Push 23 closes the remaining authorisation case: the lineage may be real and content-consistent, yet still be **foreign to the submitting customer**.

## Acceptance gate state
`npm run test:wf5-acceptance` reports **26 failing checks across only 2 returned lanes (WF1 and WF4)**.
...
### WF4 / shared-platform defects — 22
...
23. `WF4-QUOTE-FINALISATION-AUTHORITY` — customer quote intake can forge formal quote status/number/issuance evidence despite unresolved gates.
24. `WF4-QUOTE-REFINALISATION-IMMUTABILITY` — already-issued quote can be silently re-finalised under changed settings.
25. `WF4-FINALISED-QUOTE-IMMUTABILITY` — customer upsert can mutate an already-finalised quote in place.
26. `WF4-SHARE-REVISION-ISOLATION` — public R0001 share leaks later unshared revision data through the live project object.
...
WF3 F34 Candidate 02 remains customer `missing`; Push 23 promotes no visual asset.
...
The negative acceptance suite keeps release blocked because server trust boundaries can bypass or mutate that lineage. Push 23 adds explicit proof that **lineage authorisation is independent of lineage existence and content consistency**: a real R0001 cannot be accepted merely because its IDs and payload are valid if the submitting customer is not authorised for the project.
...
- WF5 acceptance gate: **FAIL / EXPECTED HOLD**, 26 checks across WF1/WF4.
- Composite `npm run test:gate`: **FAIL / EXPECTED HOLD** only after its preceding suites pass and it reaches acceptance blockers.
- Implementation/data/visual diff versus Push 22: **0 added / 0 removed / 0 changed**.

Push 23 evidence is stored under `qa-evidence/wf5-push23/`.

## Change isolation
Expected Push 23 changes are QA/package/reporting only:
- `tests/wf5-alpha26-acceptance-gate.js` — targeted `WF4-QUOTE-LINEAGE-OWNERSHIP` negative test;
...
- `PUSH_53_WF5_QA_GATE_REPORT.md` — this report; and
- `qa-evidence/wf5-push23/*` — verification logs/evidence.

## Next QA dependency
**WF4 formal quote authority + immutable lineage remains the next QA dependency, now with customer/project ownership authorisation alongside exact revision existence as mandatory admission checks.** Quote intake and formal finalisation must resolve the real immutable project/revision server-side, prove the submitting customer is authorised for that project, canonicalise commercial/BOM/render/gate data against the exact revision, and then make the governed staff finalisation transition the sole authority that can mint terminal/idempotent issuance evidence. Administrator finalisation must not retroactively legitimise a quote that crossed customer intake with foreign lineage.
