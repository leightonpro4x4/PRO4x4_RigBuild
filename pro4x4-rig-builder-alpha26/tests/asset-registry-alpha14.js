...
(async()=>{
  ...
  try{
    ...
    candidate.status='production-ready';candidate.governance={state:candidate.layerId==='base'||candidate.layerId==='wheels'?'master-approved':'layer-approved',reviewedBy:'Asset Test',reviewedAt:new Date().toISOString()};candidate.approval={...candidate.approval,state:'approved-production',approvedBy:'Asset Test',approvedAt:new Date().toISOString()};
    x=await req('PUT',`/api/v1/staff/render-assets/${encodeURIComponent(candidate.assetId)}`,candidate);assert.equal(x.r.status,422);assert.equal(x.data.code,'asset_gate_blocked');
